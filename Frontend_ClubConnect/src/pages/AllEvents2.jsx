...
const AllEvents = () => {
  ...
  const [deletingEvents, setDeletingEvents] = useState({}); // Track deleting state for each event
  ...
  const handleDeleteEvent = async (eventId) => {
    try {
      ...
      setDeletingEvents(prev => ({ ...prev, [eventId]: true }));
      ...
      setDeletingEvents(prev => ({ ...prev, [eventId]: false }));
    }
  };
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map((event) => (
            ...
            disabled={deletingEvents[event._id]}
            className={`w-full sm:w-auto ${
              deletingEvents[event._id] 
                ? 'bg-gray-400' 
                : 'bg-red-600 hover:bg-red-700'
            } text-white px-4 py-2 rounded-lg transition`}
          >
            {deletingEvents[event._id] ? 'Deleting...' : 'Delete'}
          </button>
                      <button
